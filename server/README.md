...
- **What "the owner is notified" actually means, said in the editor.** [FEATURES.md](../FEATURES.md) §3 promises the owner is notified, and the editor used to repeat it flatly under the review-date field. The sweep does write a `review_due` notice to the outbox for every flip — that part was always true — but with no `CANON_SMTP_URL` the dev transport logs it to the server's console, and Canon has **no inbox screen**: `GET /notifications` exists and nothing in the web UI calls it. So an owner with eight stale pages saw nothing, anywhere, and the editor had told them they would. Notification integrations are in FEATURES.md's **Later** tier and this round did not move them, so the promise was **narrowed to what is true** rather than made true: with a relay configured the editor says the owner is emailed, and without one it says the notice is written to the record. **The second half of that sentence is now shorter, because the queue reads the outbox** (below): the notice reaches the owner on the screen they check, whether or not a relay carries it any further. The start-up log says the same thing to the operator.
- **The queue.** `GET /queue` and `#/queue`: everything the record is waiting on the **asking** actor for, across every collection they belong to, with the count beside the nav entry. It is the answer to the one thing two user testers independently called the biggest problem with the product — a Director of Compliance who found his 25 pending approvals by opening 44 pages one at a time to read a field no listing showed, and a contributor who learned by accident that a conflict had been asserted against a policy she owns ([USER-TESTING.md](../USER-TESTING.md) T2.1). Six strands, each one an act the server already has: pages In Review this actor can approve, drafts sent back to them, pages they own that are past review, `conflicts_with` assertions against a page they own, open source divergences on one, and their own drafts in progress. Three of those became members on the query filter rather than a new read — `awaitingApprovalBy`, `sentBackTo`, `draftHeldBy` — and **`awaitingApprovalBy` is the read-side mirror of `approve`, clause for clause**: the **draft's** approver (never `pages.approver_id`, which is the approver of the *published* version and answers a different question), any `approve`-role holder where the type names nobody, and the `approve` role on every type. A queue that listed work the server would refuse is worse than no queue. **The queue writes no query of its own**: every strand is a read whose `SELECT` already joins `collection_members` for the asker, because the one screen everybody opens every morning is the worst possible place for a second permission path. There is no `?actor=` and no `/queue/:id` — the subject is whoever is asking — and the route is deliberately unclassified in `agentauth.ts`, so a passport is refused at the door. The **notification outbox is rendered here**, last and outside the count: it has no read state, so a badge counting notices would be a number that never goes down.
...
- **Concentration of duty.** One member of record health that is not a count: `approvalConcentration`, which answers the question an auditor asks about a register once she can see who approved what — *is it acceptable that these people granted every Canonical mark here?* ([USER-TESTING.md](../USER-TESTING.md) T4.8, on re-review). Four facts the record already held and nobody had assembled. **Who granted each mark, and how many each**, read from the `page.approve` event and not from `pages.approver_id` — that column is the person *named* to approve, and on a Plan, which requires no approver, `approve` accepts any holder of the role, so the two can differ and the column can be empty over a mark somebody plainly granted. **Out of how many could have**: the collection's own roster of `approve`-or-better, so "two of two" and "two of nine" are visibly different findings, with the people who hold the role and have never used it named rather than rounded away. **Where the authority came from**: `orgrole.ts` keeps hand grants and directory-group grants in separate tables precisely so *"this concentration follows from your group mapping"* and *"somebody added these two by hand"* are distinguishable sentences — the group's **name** follows the reader, since naming somebody else's group is already an operator's question, and its absence is declared in `groupsWithheld` rather than left as a blank. **And whether the approver also put the work forward**, compared against the `page.submit` that preceded each approval — which found the one case Canon's own submit-time refusal does not cover, a Plan submitted and approved by the same person, named as a gap in the control rather than reported as a breach. **There is no score.** No index, no ratio, no red/amber/green: weighing "two of nine" against "granted by hand" would be a policy judgement invented on behalf of an organisation that never made it, and an auditor handed such a number has to reverse-engineer it before she can trust it. Counts, names, origins, and a sentence saying what was counted — the same shape `backdatedWithoutBasis` takes, and every number in it opens. It is carried in the **register attestation** too, as `concentration`, inside the content digest, because the finding was raised against a bundle and belongs answered in one. See [`src/concentration.ts`](src/concentration.ts).
...
Run it from cron and from the job that verifies your backups. Run the anchor beside it:

```sh
node dist/server/scripts/anchor-head.js --db canon.db >> anchors.ndjson   # or: npm run anchor:head
node dist/server/scripts/anchor-head.js --db canon.db --explain           # the same line, with prose on stderr
```

One JSON object per line: `headEventId`, `headHash`, `events`, `takenAt`, and which record's chain it belongs to. It writes nothing to the database and appends no audit event — an anchor written *into* the log would move the head it reports — so it is safe against a live record.

**The useful form of that command is the one with a `>>` in it, and the useful place for the `>>` is another machine.** A chain in the same file as the data proves internal consistency, and an attacker who can write to the file restores internal consistency after deleting an event; an anchored head cannot be reproduced by a recomputed chain, so everything up to the last *retained* anchor becomes genuinely immutable rather than merely consistent. A line Canon wrote to Canon's own log or Canon's own disk is not retained in that sense. Where you anchor — a write-once bucket, a counter-signed mail to the compliance owner, a customer-held file, a transparency log — is your choice and not Canon's.

### Comparing two retained attestations

```sh
node dist/server/scripts/compare-attestations.js retained.json fresh.json   # 0 clean, 1 differences, 2 unreadable
```

The check that survives an attacker with write access to the record, and the only one in the product that does. Every attestation carries the chain head at the moment it was generated, so a bundle somebody kept is an external anchor in their own custody. This compares one against a fresh one — two files, no database, no network, no running Canon — checks each against its own content digest and its own chain links, and then names every difference an append-only record cannot make: a deleted event, an actor that changed (with both values), a hash recomputed over unchanged content, an event moved into the past, a rewritten version, an approval a register lost. Differences that have honest explanations — a different generator, a different `at`, new events since — are reported separately as questions and as growth. It reports and does not adjudicate: it cannot know which copy is honest, and the person who has held one since the day it was generated does.
...
POST   /imports                                    { source, path, collectionId, type?, runId?, fields? }
                                                   fields is { ownerId?, approverId?, reviewDate? }, asked once
                                                   per run and applied where the TYPE carries them -> run summary
POST   /imports/upload?source=&collectionId=&type=&runId=&ownerId=&approverId=&reviewDate=
                                                   the same importer, fed the archive itself as the body
GET    /imports                                    runs in collections the caller belongs to
GET    /imports/:id                                one run, with a per-file outcome for each file
GET    /gaps?status=                               -> { scope, collections, gaps }: questions the record
                                                   refused. An operator reads every gap; a collection's
                                                   administrator reads the ones asked of the collections they
                                                   administer, and `scope` says which list this is
POST   /gaps/:id/close                             { outcome: resolved | dismissed, note } — note required either way
POST   /access-requests                            { ground: 'collection' | 'relation', collectionId? | relationId?,
                                                   role?, note } — asking for access FROM A REFUSAL. Never a page
                                                   id: the ground is verified against what the asker was already
                                                   shown, so this cannot answer "does that exist?"
GET    /access-requests[?mine=true&status=]        the inbox (what waits on you as an administrator), or your own
                                                   requests — which name no collection and no page on a ground
                                                   you were refused
POST   /access-requests/:id/decide                 { outcome: granted | declined, role?, note } — a decline
                                                   requires the note; the grant goes through setMember
POST   /access-requests/:id/withdraw               the asker takes it back
...
GET    /collections/:id/health?draftDays=   -> { pastReview, needsUpdate, withoutOwner, orphaned, staleDrafts,
                                                 ...
                                                 approvalConcentration, pages, truncated }
                                            `approvalConcentration` -> { population, marks, granters, dormant,
                                                 eligible, selfApproved, separationChecked, separationUnknown,
                                                 marksWithoutApprovalEvent, grantersWithoutTheRoleNow,
                                                 groupsWithheld, headline, notes, limits, truncated }
                                            Who granted the Canonical marks standing here, out of how many
                                            people hold a role permitting it, where each one's role came
                                            from (a hand grant or a directory group), whether any of them
                                            also administers the collection, and whether any mark was
                                            approved by the person who submitted it. There is no score.
GET    /audit/verify?limit=                 walk the audit hash chain; requires admin on a collection
                                            -> { ok, okMeans, format, algorithm, recipe, events,
                                                 chainedFromEventId, unchained, verified, partial,
                                                 head: { eventId, hash },
                                                 firstBreak: { kind, eventId, at, action, expected, found,
                                                               explanation } | null,
                                                 proves, limits, externalAnchor }
                                            `okMeans` travels with `ok` deliberately: a log whose links have
                                            been recomputed wholesale answers `ok: true`, which is the right
                                            answer to what this checks and the wrong sentence to read alone.
...
GET    /collections/:id/attestation?at=&format=
                                            ...
                                            instant, with owners, the approver NAMED on each and the person
                                            who actually GRANTED it, approval dates and review dates — and
                                            which did not, named rather than omitted. It carries the
                                            concentration-of-duty section as `concentration`, inside the
                                            content digest.
...
### The screen, and the two doors

Everything below is also **a screen**: `#/imports` lists every run in the collections you belong to, opens
one to show the outcome of every file it read and why any of them failed, and runs it again. The import
itself has two doors, and the screen offers both — **upload a `.zip`**, which is the ordinary path and needs
nothing but the export file, or **name a folder on the Canon server**, which is for a corpus already sitting
beside the record. The rest of this section describes the second door and the API under both.
...
The directory must be readable by the Canon server process, on the server's own filesystem — `path` is a
server-side path. `POST /imports/upload` takes the archive itself instead (the parameters ride the query
string, because the body is the corpus); it unpacks into a spool the server owns, runs the same importer, and
removes the spool whether the run succeeded or was refused. Unpacking by hand is the path for a corpus that is
already on the server.

### 2. Choose a collection, a type, and who will own the result

Create (or pick) the collection the material belongs in; the importer requires **admin** on it (see
[Importing: where an import may read from](#importing-where-an-import-may-read-from) — a run names a
server-side path and lands up to 2000 pages in one call, which is administration rather than authoring).
Then choose the document type every imported page will carry. `note` is the default and the right answer for a
bulk first import: Notes publish directly, so each page arrives with its body as version 1 and is immediately
readable and searchable.

A run also names, **once**, who will own what it lands: an `owner`, an `approver` and a `review date`, each
applied where the chosen type carries one. Naming a field the type does not carry is refused with the type
named rather than quietly dropped, and the owner defaults to the person running the import. This is the answer
to a migration that silently produces content accountable to nobody.

One field is deliberately **not** askable: the **effective date**, the day a policy began to apply. It is a
fact about each document and different for every one of them, and one date stamped across a corpus would be a
fabrication the record would then serve, cite and attest. So a Policy import still cannot publish; its body
waits in the page's draft, and the run says which field each file is short of.
...
curl -sS -X POST http://localhost:3000/imports \
  ...
  -H "x-actor-id: $ACTOR" \
  -d '{"source":"confluence","path":"/home/ops/exports/member-benefits","collectionId":"'"$COLLECTION"'",
       "type":"spec","fields":{"ownerId":"'"$OWNER"'","approverId":"'"$APPROVER"'","reviewDate":"2027-03-01"}}'
```

`source` is `confluence` or `google-docs`. `type` defaults to `note`. `fields` is optional; what it may carry
follows the type. The response is the run summary:
...
```jsonc
{
  ...
  "fields": { "ownerId": "…", "approverId": "…", "reviewDate": "2027-03-01" },  // as applied, on every page
...
- **Attribution, and accountability.** The actor who ran the import is the creator of every page and the author of every version it writes. Every page also carries the owner, approver and review date the RUN named, asked once and recorded on the run itself — so a re-run repeats them and the audit log's `import.start` event says who was made accountable for a whole corpus, and by whom. Where nothing was named, the importer is the owner; an approver is never assumed.
